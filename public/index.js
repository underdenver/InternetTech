...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите название товара:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое название товара', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Товар '${taskName}' будет удален. Продолжить?`)) return;
